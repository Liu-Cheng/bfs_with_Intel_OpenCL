...
int main(int argc, char ** argv){
	...
	status = clEnqueueSVMMap(queues[0], CL_TRUE, CL_MAP_READ, (void *)rpaInfo, sizeof(int) * 2 * vertexNum, 0, NULL, NULL); 
	if(status != CL_SUCCESS) ERROR("Failed clEnqueueSVMMap, error code: %d", status);
	clSetKernelArgSVMPointerAltera(kernels[0], 0, (void*)rpaInfo);

	status = clEnqueueSVMMap(queues[1], CL_TRUE, CL_MAP_READ, (void *)cia, sizeof(int) * ciaoSize, 0, NULL, NULL); 
	if(status != CL_SUCCESS) ERROR("Failed clEnqueueSVMMap, error code: %d", status);
	clSetKernelArgSVMPointerAltera(kernels[1], 0, (void*)cia);
	
	status = clEnqueueSVMMap(queues[2], CL_TRUE, CL_MAP_WRITE, (void *)nextFrontierSize, sizeof(int) * batch, 0, NULL, NULL); 
	if(status != CL_SUCCESS) ERROR("Failed clEnqueueSVMMap, error code: %d", status);
	clSetKernelArgSVMPointerAltera(kernels[2], 0, (void*)nextFrontierSize);
	clSetKernelArg(kernels[2], 1, sizeof(int), (void*)&rootVidx);
#ifdef SW_EMU
	for(int i = 0; i < batch; i++){
		status = clEnqueueSVMMap(queues[2], 
		...
		status = clSetKernelArgSVMPointerAltera(kernels[2], 3 + i, (void*)(bmap[i]));
		if(status != CL_SUCCESS) ERROR("set argument error.");
	}
#endif
	
	for(int i = 0; i < batch; i++){
		status = clEnqueueSVMMap(queues[ 3 + i], CL_TRUE, CL_MAP_WRITE, (void *)(nextFrontier[i]), sizeof(int) * segSize, 0, NULL, NULL); 
		if(status != CL_SUCCESS) ERROR("Failed clEnqueueSVMMap, error code: %d", status); 
		status = clSetKernelArgSVMPointerAltera(kernels[3 + i], 0, (void*)(nextFrontier[i]));
		if(status != CL_SUCCESS) ERROR("set argument error.");
	}
	...
	while(frontierSize > 0){
		...
		clSetKernelArg(kernels[0], 1, sizeof(int), (void*)&frontierSize);
		clSetKernelArg(kernels[1], 1, sizeof(int), (void*)&frontierSize);
		clSetKernelArg(kernels[2], 2, sizeof(char), (void*)&level);
		...
		for(int i = 0; i < batch; i++){
			status = clEnqueueTask(queues[3 + i], kernels[3 + i],  0, NULL, NULL);
			if(status != CL_SUCCESS) ERROR("Failed to enqueue task.");
		}
		...
		for(int i = 1; i < batch; i++){
			baseVec[i] = baseVec[i - 1] + 2 * nextFrontierSize[i - 1];
		}
	...
	std::cout << "Measured MTEPS: " << 1000 * millionEdges / elapsedTime << std::endl;
	std::cout << "FPGA execution in each iteration: ";
	double sum = 0;
	...
	std::cout << std::endl;
	std::cout << "Total FPGA exeuction time: " << sum << " ms" << std::endl;
